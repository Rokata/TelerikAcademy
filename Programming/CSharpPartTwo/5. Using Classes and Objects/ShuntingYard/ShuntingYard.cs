...
public class ShuntingYard
{
    ...
    private static decimal ExtractNum(int offset, int startPoint, StringBuilder expression)
    {
        ...
        startPoint += offset;
        if (expression[startPoint] == '-' || expression[startPoint] == '+')
        {
        ...
        bool check = expression[startPoint + offset] == '*' || expression[startPoint + offset] == '/' || expression[startPoint + offset] == '+' || expression[startPoint + offset] == '-';
        if (offset == -1 && ((expression[startPoint] == '+' && check) || (expression[startPoint] == '-' && check)))
        {
            result.Insert(0, expression[startPoint]);
        }
